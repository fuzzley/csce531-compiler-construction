...
$submissionRoot = "$ENV{HOME}/courses/csce531/sp14/submissions";
$testFilePrefix = 'T1L';
$hwDir = 'pas-proj1';
...
#sub main
{
	...
	while (@ARGV) {
	    $uname = shift @ARGV;
	    $proj1_dir = "$submissionRoot/$uname/$hwDir";
	    process_user();
	}
    }
    else {
...
sub process_user {
    ...
    die "No proj1 directory corresponding to $uname ($!)\n"
	unless -d $proj1_dir;

    open(COMMENTS, "> $proj1_dir/comments.txt");

    cmt("Comments for $uname -------- " . now() . "\n");

    chdir $proj1_dir
	or die "Cannot change to directory $proj1_dir ($!)\n";

    # try make clean, regardless of what happens
    system("make", "clean");

    opendir DIR, $proj1_dir
	or die "Cannot open $proj1_dir directory ($!)\n";
